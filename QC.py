...
input_csv = pd.read_csv('Data Transfers/Study_PROD_FULL_yyyymmdd1.csv', usecols=['LBREFID', 'LBTESTCD', 'LBORRES']).drop_duplicates(keep='first')
other_columns_csv=pd.read_csv('Data Transfers/Study_PROD_FULL_yyyymmdd1.csv', usecols=['USUBJID', 'LBREFID', 'VISIT', 'LBDTC', 'LBREASND']).drop_duplicates(keep='first')
duplicates_csv=pd.read_csv('Data Transfers/Study_PROD_FULL_yyyymmdd1.csv', usecols=['USUBJID', 'LBREFID'])

#Change name of Abeta file
input_abeta_csv=pd.read_csv('Lab Data/All Abeta Data_ddmmmyyyy.csv', usecols=['Plasma Barcodes', 'A?40 (pg/mL)', 'A?42 (pg/mL)', 'A?42/40', 'Comments', 'Date of collection', 'SubjectID']).drop_duplicates(keep='first')

#Change name of ApoE File
input_apoe_csv=pd.read_csv('Lab Data/All ApoE Data_ddmmmyyyy.csv', usecols=['Plasma Barcodes', 'Proteotype']).drop_duplicates(keep='first')
...
full_file.to_csv('QC Files/Study_PROD_FULL_yyyymmdd1_QC.csv', index=False)
